    ...
    public class WebSocketServer
    {
        ...
        private void ShakeHands(Socket conn)
        {
            ...
            using (var writer = new StreamWriter(stream))
            {
                ...
                string tmp = new System.Text.RegularExpressions.Regex("Sec-WebSocket-Key: (.*)").
                             Match(data).Groups[1].Value.Trim();


                tmp = tmp.Split('=')[0] + "==" + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";

                tmp = Convert.ToBase64String(
                                 System.Security.Cryptography.SHA1.Create().ComputeHash(
                                 Encoding.UTF8.GetBytes(
                                 tmp)));

                writer.WriteLine("Sec-WebSocket-Accept: " + tmp);
